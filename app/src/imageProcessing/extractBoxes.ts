...
function extractSudokuBoxes(
  grayscaleImage: ImageInterface,
  thresholdedImage: ImageInterface
): SudokuBox[] {
  ...
  for (let yIndex = 0; yIndex < 9; yIndex++) {
    for (let xIndex = 0; xIndex < 9; xIndex++) {
      ...
      for (let searchY = searchY1; searchY < searchY2; searchY++) {
        for (let searchX = searchX1; searchX < searchX2; searchX++) {
          if (thresholdedImage.bytes[searchY * imageSize + searchX] === 255) {
            ...
            const component = findBlob(thresholdedImage, searchX, searchY);
            const foundWidth =
      ...
      if (
        ...
        const numberImage = grayscaleImage.createSubImage(
          Math.max(0, minX - 2),
          Math.max(0, minY - 2),
          Math.min(imageSize - 1, maxX + 2),
          Math.min(imageSize - 1, maxY + 2)
        );
        boxes.push({
          x: xIndex,
          y: yIndex,
...
export { extractSudokuBoxes, SudokuBox };
