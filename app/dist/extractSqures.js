    ...
    for (let y = 0; y < 9; y++) {
        for (let x = 0; x < 9; x++) {
            ...
            for (let searchY = searchY1; searchY < searchY2; searchY++) {
                for (let searchX = searchX1; searchX < searchX2; searchX++) {
                    if (thresholded.bytes[searchY * size + searchX] === 255) {
                        const component = (0, largestObjectLocalisation_1.findBlob)(thresholded, searchX, searchY);
            ...
            if (pointsCount > 10 &&
                ...
                const numberImage = greyScale.createSubImage(Math.max(0, minX - 2), Math.max(0, minY - 2), Math.min(size - 1, maxX + 2), Math.min(size - 1, maxY + 2));
                results.push({
                    x,
                    y,
                    minX,
...
exports.default = extractBoxes;
//# sourceMappingURL=extractSqures.js.map